...
int liblogging_debugLevels[LIBLOGGING_MAX_SESSION_NUM];
/*
 * Each thread has a dedicated buffer to buffer logging content.
 * The logging agent fetches logging content from these buffers.
 */
liblogging_buf_t lbuf = {0,};

#define LIBLOGGING_INSUFFICIENT_SPACE ((size_t)-1)
#define LIBLOGGING_ALIGN(_offset, _aligned_size) \
...
typedef struct{
    ...
    uint16_t string_off[LIBLOGGING_MAX_ARG_NUM];
    uint16_t size;
#endif
    uint16_t string_len[LIBLOGGING_MAX_ARG_NUM];
    const char* string[LIBLOGGING_MAX_ARG_NUM];
}va_list_inf_t;

static void initialize_thread_buffers() {
    int i;
    pthread_t log_agent_thread;
    lbuf.size = LIBLOGGING_THREAD_BUFFER_SIZE;
    lbuf.buf = (uint8_t*) malloc(
            sizeof(uint8_t) * lbuf.size);
    lbuf.read_block_cnt = 0;
    lbuf.read_offset = 0;
    lbuf.read_block_index = 0;
    lbuf.write_offset = sizeof(liblogging_block_header_t);
    lbuf.read_offset = sizeof(liblogging_block_header_t);
    for (i = 0; i < LIBLOGGING_THREAD_BUFFER_BLOCK_NUM; i++) {
        liblogging_block_header_t* cur_block =
                (liblogging_block_header_t*) (lbuf.buf + i
                        *LIBLOGGING_THREAD_BUFFER_BLOCK_SIZE
                        * sizeof(uint8_t));
        liblogging_content_header_t* first_content_header =
                (liblogging_content_header_t*) ((uint8_t*) cur_block
                        + sizeof(liblogging_block_header_t));
        ...
        first_content_header->lunit = NULL;//LIBLOGGING_INVALID_UNIT_ID;
        //printf("%d %p\n", i, cur_block);
    }
    lbuf.read_cursor = (liblogging_block_header_t*)lbuf.buf;
    lbuf.init_flag = 1;

    pthread_create(&log_agent_thread, NULL, liblogging_agent_read_content, NULL);
}
...
static size_t calculate_required_sapce(
        va_list_inf_t* va_list_inf,
        logging_unit_t* lunit, va_list args){
    size_t req_size;
    ...
    req_size += sizeof(liblogging_content_header_t);
    req_size = LIBLOGGING_ALIGN(req_size, sizeof(size_t));
    va_list_inf->size = req_size;
    return req_size;
}

static size_t write_args(liblogging_content_header_t* content_header, uint8_t* buf, va_list_inf_t* va_list_inf,
        logging_unit_t* lunit, va_list args) {
    ...
    assert(va_list_inf->size == LIBLOGGING_ALIGN(sizeof(liblogging_content_header_t) + offset, sizeof(size_t)));
    if(1)
    {
        ...
        if(strcmp(testingstr, productstr)){
            size_t block_offset = BLOCK_OFFSET(buf - lbuf.buf);
            size_t free_space = LIBLOGGING_THREAD_BUFFER_BLOCK_SIZE - block_offset;
            printf("ERROR!\n%s\n%s\n\nblock_offset: %ld, free_space: %ld, required_size: %ld\n", testingstr, productstr, block_offset, free_space,
                    va_list_inf->size - sizeof(liblogging_content_header_t));
            //assert( LIBLOGGING_THREAD_BUFFER_BLOCK_SIZE - BLOCK_OFFSET(buf - lbuf.buf) >=  va_list_inf.size);
        }
...
#define LIBLOGGING_LOCK_BLOCK(__block_lock) __sync_fetch_and_add(__block_lock, 2)
#define LIBLOGGING_UNLOCK_BLOCK(__block_lock) __sync_fetch_and_add(__block_lock, -2)
static uint8_t* start_with_new_block(
        size_t** lock,
        liblogging_buf_t* lbuf,
        size_t required_space_size,
        size_t free_space_size,
        size_t write_offset
        ) {
    uint8_t* buf;
    liblogging_block_header_t* block_header;
    size_t write_offset_updated;
    write_offset_updated = write_offset + free_space_size;

    block_header = (liblogging_block_header_t*)(lbuf->buf + BOUNDED_OFFSET(write_offset_updated));
    ...
    write_offset_updated += sizeof(liblogging_block_header_t);
    buf = lbuf->buf + BOUNDED_OFFSET(write_offset_updated);
    write_offset_updated = write_offset_updated + required_space_size;
    LIBLOGGING_LOCK_BLOCK(&block_header->lock);
    /*To do, ABA problem impact analyze ?*/
    if(__sync_bool_compare_and_swap(&lbuf->write_offset, write_offset, write_offset_updated)){
        block_header->write_sequence_num = SEQUENCE_NUM(write_offset_updated);
        if(free_space_size >= sizeof(liblogging_content_header_t)){
            liblogging_content_header_t* content_header =
                    (liblogging_content_header_t*)(lbuf->buf + BOUNDED_OFFSET(write_offset));
            content_header->size = 0;
        }
        if((block_header->write_sequence_num & 0xffff) == 0)
        ...
        *lock = &block_header->lock;
        ...
        LIBLOGGING_UNLOCK_BLOCK(&block_header->lock);
        return NULL;
    }
}

static uint8_t* get_space_from_log_buffer(size_t** lock,
        va_list_inf_t* va_list_inf,
        logging_unit_t* lunit, va_list args, liblogging_buf_t* log_buf) {
    uint8_t* buf;
    size_t required_space_size;
    size_t block_offset;
    size_t free_space_size;
    ...
    if(required_space_size > LIBLOGGING_THREAD_BUFFER_BLOCK_LOAD_SIZE)
    ...
    do{
        ...
        free_space_size =
                LIBLOGGING_THREAD_BUFFER_BLOCK_SIZE - block_offset;

        if( required_space_size + write_offset - log_buf->read_offset > LIBLOGGING_THREAD_BUFFER_SIZE ){
            /*Issue 1, to do, important log (ERROR, WARNING) should be kept and drops non-important (INFO) logs.*/
        ...
        write_secquence_unm_expected = SEQUENCE_NUM(write_offset - 1);
        block_header = (liblogging_block_header_t*)(log_buf->buf + CAST_OFFSET(write_offset));
        ...
        if (required_space_size > free_space_size) {
            ...
            buf = log_buf->buf + BOUNDED_OFFSET(write_offset);
 ...
 * 2 Allocate space from logging buffer.
 *      if block.busy > 0
 *          if
...
int liblogging_write_arguments(logging_unit_t* lunit, const char* __restrict fmt, ...) {
    liblogging_content_header_t* content_header;
    liblogging_buf_t* log_buf = &lbuf;
    va_list_inf_t va_list_inf;
    size_t* lock;
    va_list args;
    va_start(args, fmt);
    ...
    if(buf == NULL)
        return 1;
    content_header = (liblogging_content_header_t*)buf;
    write_args(content_header,
            buf + sizeof(liblogging_content_header_t),
            &va_list_inf,
            lunit, args);

    LIBLOGGING_UNLOCK_BLOCK(lock);
    va_end(args);
...
static int initialize_debugLevels() {
    int i;
    for (i = 0; i < LIBLOGGING_MAX_SESSION_NUM; i++)
        liblogging_debugLevels[i] = LIBLOGGING_DEFAULT_DEBUG_LEVEL;
    return i;
}

int set_debug_level(int session, int level) {
    if (session >= LIBLOGGING_MAX_SESSION_NUM)
        return -1;
    liblogging_debugLevels[session] = level;
    if (level > LIBLOGGING_MAX_DEBUG_LEVEL)
        level = LIBLOGGING_MAX_DEBUG_LEVEL;
    return level;
}

static uint16_t get_arguments_size_by_type(logging_unit_t* lunit, int type, uint8_t* new_type) {
...
#define BASE_TYPE_NUM 8
    ...
    case PA_FLOAT:
        if (IS_LONG_DOUBLE(type)) {
            *new_type = LIBLOGGING_LONG_DOUBLE;
#ifdef __x86_64__
            lunit->size_long_double += 16;
#endif
            size = sizeof(long double);
        } else {
            *new_type = LIBLOGGING_DOUBLE;
#ifdef __x86_64__
...
static uint16_t calculate_arguments_total_size(logging_unit_t* lunit, int* arg_type) {
...
#endif
            if(i > 0)
                total_size = LIBLOGGING_ALIGN(total_size, size);
            if(LIBLOGGING_STRING == lunit->arg_type[i]){
#ifdef __x86_64__
...
#ifdef __x86_64__
            if( LIBLOGGING_DOUBLE != lunit->arg_type[i] &&
                LIBLOGGING_LONG_DOUBLE != lunit->arg_type[i] &&
                LIBLOGGING_FLOAT != lunit->arg_type[i])
                non_float_num += 1;
            else
                float_num += 1;
#endif
...
void initialize_logging_unit_t(logging_unit_t* lunit, const char* __restrict file,
        const char* __restrict fun, int line, const char* __restrict fmt, ...) {
    va_list args;
    va_start(args, fmt);
    static size_t base_id = 0;
    int arg_type[LIBLOGGING_MAX_ARG_NUM];
    lunit->format_str = fmt;
    lunit->id = __sync_fetch_and_add(&base_id, 1);
    if (lunit->id == 0) {
        /*Initialize the logging library as this is the first call of it.*/
